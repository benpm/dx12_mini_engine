...
* **Per-entity**: `orbit.lua` (Y-axis orbit), `bounce.lua` (vertical bounce with squash/stretch), `pulse_emissive.lua` (pulsing glow), `character_controller.lua` (WASD-driven physics character — set body horizontal velocity from input each frame, preserve gravity Y)
* **One-shot actions**: `spawn_grid.lua` (5x5 entity grid), `randomize_colors.lua` (random albedo on all MeshRef entities), `delete_all.lua` (destroy all MeshRef entities), `physics_demo.lua` (static floor + stack of dynamic boxes attached to mesh entities via Jolt), `physics_stress.lua` (1000-entity convex-hull stress — 10×10×10 grid of dynamic bodies; hulls built from `engine.add_convex_hull_body` sub-sampling mesh positions; teapots/plane/torus filtered for stability), `character_demo.lua` (arena scaffolding for `character_controller.lua` — static floor body, 4 cube obstacles, sphere visual on capsule body, looks up "Cube"/"Sphere" by name via `engine.get_mesh_names`, attaches `character_controller.lua` to the player via `engine.attach_script`)
...
* **Scenes**: `physics_stress.json` boots the 1000-body stress automatically via `startupScript`. Camera and lighting are tuned to view the full 30×30 m falling grid. `character_demo.json` boots `character_demo.lua` for the WASD character controller demo (screenshotFrame=30, no auto-exit so the demo stays interactive).

## Bundled primitive naming

Primitive `.glb` files in `resources/models/` (cube, sphere, cylinder, cone, plane, pyramid, capsule, icosphere, torus) have empty `mesh.name` fields, so `Scene::loadGltf` falls back to `GLB mesh N`. `Application::loadContent` post-processes these after the `MODELS_DIR` sweep: each newly-added mesh whose name starts with `GLB mesh` is renamed to the source filename stem with the first letter capitalised (`cube.glb` → `Cube`). Lua scripts can therefore do `engine.get_mesh_names()` + `string.lower(name) == "cube"` lookups instead of relying on alphabetical load order — see `character_demo.lua`'s `find_mesh` helper.
