...
| `physics.ixx` | `PhysicsWorld` class — backend-agnostic facade that forwards every call to an `IPhysicsBackend` (interface in `include/physics_backend.h`). Active backend is chosen at compile time via the CMake option `ENGINE_PHYSICS_BACKEND` (`Jolt` default, `PhysX` opt-in, `None` for no-op). Steps in `Application::update()` ahead of `scene.progress(dt)`, immediately followed by a `RigidBody → Transform` sync (full translation + quaternion rotation, entity scale preserved by extracting it from the existing transform). Body API: `createBoxBody`, `createSphereBody`, **`createCapsuleBody(px, py, pz, halfHeight, radius, ...)`**, **`createConvexHullBody(points, count, stride, ...)`**, `destroyBody`, `getBodyPosition`, `getBodyRotation`, `getLinearVelocity` / `setLinearVelocity`, `getAngularVelocity` / `setAngularVelocity`, `applyForce`, `applyImpulse`, `setBodyPosition`, `raycast`. Lua bindings: `engine.add_box_body`, `engine.add_sphere_body`, **`engine.add_capsule_body(x, y, z, halfHeight, radius, dynamic, mass)`** (Y-up, defaults to a 1.8 m / 70 kg human shape), **`engine.add_convex_hull_body(meshIdx, x, y, z, scale, dynamic, mass, maxPoints, tolerance)`**, **`engine.add_mesh_collider(entity, dynamic, mass, maxPoints, tolerance)`** (convenience: looks up the entity's `MeshRef` + extracts position/scale from its `Transform`, builds a hull body, attaches it via `RigidBody` — three calls collapsed into one), `engine.remove_body`, `engine.get_body_position`, `engine.get_body_rotation`, `engine.get_linear_velocity` / `engine.set_linear_velocity`, `engine.get_angular_velocity` / `engine.set_angular_velocity`, `engine.apply_force`, `engine.apply_impulse`, `engine.set_body_position`, `engine.raycast`, `engine.attach_rigid_body(entity, body)`, **`engine.get_rigid_body(entity)`** (returns the entity's attached body id, or 0).

**Input bindings:** `engine.is_button_down(name)` polls the engine's gainput-driven input map. Recognised button names: `MoveForward`, `MoveBackward`, `MoveLeft`, `MoveRight`, `LeftClick`, `RightClick`, `ScrollUp`, `ScrollDown`, `Exit`. The default keyboard mapping (WASD + arrows) lives in `Application`'s input setup — script bindings reuse it. See `resources/scripts/character_controller.lua` for a per-entity script that drives a capsule via these. The hull builder sub-samples mesh positions to `maxPoints` (default 32) then hands them to Jolt with `hullTolerance` for further simplification; hard cap at Jolt's `cMaxPointsInHull=256`. ECS component `RigidBody{ bodyId }` binds an entity to a body; see `resources/scripts/physics_demo.lua`. |
...
- P15 ✅ (rolling) — Remaining raw `ComPtr<ID3D12Resource>` upload/readback buffers migrate to `gfx::BufferHandle` (`BufferUsage::Upload` / `BufferUsage::Readback`, mapped via `gfxDevice->map/unmap`). Done so far: `Application::occlusionReadback`, `SsaoRenderer::noiseUploadBuf`. Each migration drops a `CreateCommittedResource(HEAP_TYPE_UPLOAD/READBACK)` block in favour of a `BufferDesc` + `createBuffer` call; native pointer access (for `CopyTextureRegion`, `ResolveQueryData`, etc.) goes through `gfxDevice->nativeResource(handle)`.
