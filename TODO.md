...
- [ ] **Reorganization:** Move shader files to their own directory in src/ . For source files like `application_scene.cpp` and `application_setup.cpp`, which are parts of `Application`, make a subdir in src/ for them. So `application_scene.cpp` would become `application/scene.cpp`.
