...
SwapChain::SwapChain(Device* dev, const gfx::SwapChainDesc& desc)
    : device(dev), hwnd_(reinterpret_cast<HWND>(desc.nativeWindowHandle)), width_(desc.width),
      height_(desc.height), bufferCount_(desc.bufferCount), format_(desc.format),
      allowTearing_(desc.allowTearing)
{
    ...
    chkDX(dev->dxgiFactory()->CreateSwapChainForHwnd(queueNative, hwnd_, &sd, nullptr, nullptr,
                                                     &sc1),
          "CreateSwapChainForHwnd");
    chkDX(sc1.As(&swap), "SwapChain QI IDXGISwapChain4");
    chkDX(dev->dxgiFactory()->MakeWindowAssociation(hwnd_, DXGI_MWA_NO_ALT_ENTER),
          "MakeWindowAssociation");

    createBackBufferTextures();
}

SwapChain::~SwapChain() { releaseBackBufferTextures(); }
...
{
    for (auto h : backBufferHandles) device->releaseBackBuffer(h);
...
void SwapChain::resize(uint32_t w, uint32_t h)
{
    ...
    chkDX(swap->ResizeBuffers(bufferCount_, w, h, toDXGI(format_), flags),
...
uint32_t SwapChain::currentIndex() const { return swap->GetCurrentBackBufferIndex(); }
