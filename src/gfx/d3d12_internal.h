...
inline D3D12_RESOURCE_STATES toD3D12States(gfx::ResourceState s)
{
    ...
    if (has(S::IndexBuffer)) out |= D3D12_RESOURCE_STATE_INDEX_BUFFER;
    if (has(S::RenderTarget)) out |= D3D12_RESOURCE_STATE_RENDER_TARGET;
    if (has(S::UnorderedAccess)) out |= D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
    if (has(S::DepthWrite)) out |= D3D12_RESOURCE_STATE_DEPTH_WRITE;
    if (has(S::DepthRead)) out |= D3D12_RESOURCE_STATE_DEPTH_READ;
    if (has(S::PixelShaderResource)) out |= D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
    if (has(S::NonPixelShaderResource)) out |= D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
    if (has(S::CopySource)) out |= D3D12_RESOURCE_STATE_COPY_SOURCE;
    if (has(S::CopyDest)) out |= D3D12_RESOURCE_STATE_COPY_DEST;
    if (has(S::Present)) out |= D3D12_RESOURCE_STATE_PRESENT;
...
inline D3D12_PRIMITIVE_TOPOLOGY_TYPE toTopologyType(gfx::PrimitiveTopology t)
{
    switch (t) {
        ...
        case gfx::PrimitiveTopology::LineList: return D3D12_PRIMITIVE_TOPOLOGY_TYPE_LINE;
        case gfx::PrimitiveTopology::PointList: return D3D12_PRIMITIVE_TOPOLOGY_TYPE_POINT;
    }
    return D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;
}
...
{
    switch (t) {
        case gfx::PrimitiveTopology::TriangleList: return D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
        case gfx::PrimitiveTopology::TriangleStrip: return D3D_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP;
        case gfx::PrimitiveTopology::LineList: return D3D_PRIMITIVE_TOPOLOGY_LINELIST;
        case gfx::PrimitiveTopology::PointList: return D3D_PRIMITIVE_TOPOLOGY_POINTLIST;
    }
...
{
    switch (op) {
        case gfx::CompareOp::Never: return D3D12_COMPARISON_FUNC_NEVER;
        case gfx::CompareOp::Less: return D3D12_COMPARISON_FUNC_LESS;
        case gfx::CompareOp::Equal: return D3D12_COMPARISON_FUNC_EQUAL;
        case gfx::CompareOp::LessEqual: return D3D12_COMPARISON_FUNC_LESS_EQUAL;
        case gfx::CompareOp::Greater: return D3D12_COMPARISON_FUNC_GREATER;
        case gfx::CompareOp::NotEqual: return D3D12_COMPARISON_FUNC_NOT_EQUAL;
        case gfx::CompareOp::GreaterEqual: return D3D12_COMPARISON_FUNC_GREATER_EQUAL;
        case gfx::CompareOp::Always: return D3D12_COMPARISON_FUNC_ALWAYS;
    }
...
{
    switch (f) {
        case gfx::BlendFactor::Zero: return D3D12_BLEND_ZERO;
        case gfx::BlendFactor::One: return D3D12_BLEND_ONE;
        case gfx::BlendFactor::SrcColor: return D3D12_BLEND_SRC_COLOR;
        case gfx::BlendFactor::InvSrcColor: return D3D12_BLEND_INV_SRC_COLOR;
        case gfx::BlendFactor::SrcAlpha: return D3D12_BLEND_SRC_ALPHA;
        case gfx::BlendFactor::InvSrcAlpha: return D3D12_BLEND_INV_SRC_ALPHA;
        case gfx::BlendFactor::DstAlpha: return D3D12_BLEND_DEST_ALPHA;
        case gfx::BlendFactor::InvDstAlpha: return D3D12_BLEND_INV_DEST_ALPHA;
        case gfx::BlendFactor::DstColor: return D3D12_BLEND_DEST_COLOR;
        case gfx::BlendFactor::InvDstColor: return D3D12_BLEND_INV_DEST_COLOR;
    }
...
inline D3D12_BLEND_OP toBlendOp(gfx::BlendOp op)
{
    switch (op) {
        ...
        case gfx::BlendOp::Subtract: return D3D12_BLEND_OP_SUBTRACT;
        case gfx::BlendOp::ReverseSubtract: return D3D12_BLEND_OP_REV_SUBTRACT;
...
{
    switch (m) {
        case gfx::CullMode::None: return D3D12_CULL_MODE_NONE;
        case gfx::CullMode::Front: return D3D12_CULL_MODE_FRONT;
        case gfx::CullMode::Back: return D3D12_CULL_MODE_BACK;
    }
...
{
   public:
    void init(ID3D12Device2* device, uint32_t capacity, D3D12_DESCRIPTOR_HEAP_TYPE type,
...
class CommandList final : public gfx::ICommandList
{
   public:
    ...
    void setVertexBuffer(uint32_t slot, gfx::BufferHandle b, uint32_t stride,
    ...
    void barrier(gfx::TextureHandle h, gfx::ResourceState before,
                 gfx::ResourceState after) override;
    void barrier(gfx::BufferHandle h, gfx::ResourceState before,
                 gfx::ResourceState after) override;
    void uavBarrier(gfx::TextureHandle h) override;
    void copyBuffer(gfx::BufferHandle dst, uint64_t dOff, gfx::BufferHandle src, uint64_t sOff,
                    uint64_t size) override;
    void copyTextureToBuffer(gfx::BufferHandle dst, uint64_t dOff, gfx::TextureHandle src,
                             uint32_t srcMip, uint32_t srcSlice, int32_t x, int32_t y, uint32_t w,
                             uint32_t h) override;
...
class Queue final : public gfx::IQueue
{
   ...
   private:
    ...
    std::unordered_map<ID3D12GraphicsCommandList2*, Microsoft::WRL::ComPtr<ID3D12CommandAllocator>>
        listAllocs;
...
class Device final : public gfx::IDevice
{
   public:
    ...
    void uploadBuffer(gfx::BufferHandle dst, const void* data, uint64_t size,
                      uint64_t dstOffset) override;
    void uploadTexture(gfx::TextureHandle dst, const void* data, uint64_t rowPitch,
                       uint64_t slicePitch) override;
    ...
    gfx::TextureHandle adoptBackBuffer(Microsoft::WRL::ComPtr<ID3D12Resource> resource,
                                       gfx::Format format);
    void releaseBackBuffer(gfx::TextureHandle h);

    void schedulePendingDestroy(uint64_t fenceValue,
                                Microsoft::WRL::ComPtr<ID3D12Resource> resource);
