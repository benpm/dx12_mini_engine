...
Device::Device(const gfx::DeviceDesc& d) : desc(d)
{
    ...
    if (SUCCEEDED(d3dDevice->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS5, &opts5,
                                                  sizeof(opts5)))) {
        capabilities.raytracing = opts5.RaytracingTier >= D3D12_RAYTRACING_TIER_1_0;
    }
    D3D12_FEATURE_DATA_D3D12_OPTIONS7 opts7{};
    if (SUCCEEDED(d3dDevice->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS7, &opts7,
                                                  sizeof(opts7)))) {
...
{
    UINT factoryFlags = 0;
    if (desc.enableDebugLayer) factoryFlags |= DXGI_CREATE_FACTORY_DEBUG;
    ...
    if (desc.useWarp) {
        ...
        for (UINT i = 0;
            ...
            adapter->GetDesc1(&ad);
            if ((ad.Flags & DXGI_ADAPTER_FLAG_SOFTWARE) != 0) continue;
            if (SUCCEEDED(D3D12CreateDevice(adapter.Get(), D3D_FEATURE_LEVEL_12_0,
                                             __uuidof(ID3D12Device2), nullptr))) {
                break;
            }
        }
    }
    chkDX(D3D12CreateDevice(adapter.Get(), D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(&d3dDevice)),
          "D3D12CreateDevice");
}

void Device::initBindlessHeaps(uint32_t capacity)
{
    resourceHeap.init(d3dDevice.Get(), capacity, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, true,
                      L"gfx_bindless_resource_heap");
    samplerHeap_.init(d3dDevice.Get(), 256, D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, true,
...
void Device::createBindlessRootSignature()
{
    ...
    sampRange.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER, UINT_MAX, 0, 0,
                   D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE);
    params[4].InitAsDescriptorTable(1, &sampRange);

    CD3DX12_VERSIONED_ROOT_SIGNATURE_DESC rsd;
    rsd.Init_1_1(_countof(params), params, 0, nullptr,
                 D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT |
                     D3D12_ROOT_SIGNATURE_FLAG_CBV_SRV_UAV_HEAP_DIRECTLY_INDEXED |
                     D3D12_ROOT_SIGNATURE_FLAG_SAMPLER_HEAP_DIRECTLY_INDEXED);

    ComPtr<ID3DBlob> blob, err;
    HRESULT hr = D3DX12SerializeVersionedRootSignature(&rsd, D3D_ROOT_SIGNATURE_VERSION_1_1, &blob,
                                                       &err);
    if (FAILED(hr)) {
        throw std::runtime_error(std::string("Bindless root sig serialize failed: ") +
                                 (err ? static_cast<const char*>(err->GetBufferPointer()) : ""));
    }
    chkDX(d3dDevice->CreateRootSignature(0, blob->GetBufferPointer(), blob->GetBufferSize(),
...
{
    D3D12_RESOURCE_DESC rd = CD3DX12_RESOURCE_DESC::Tex2D(toDXGI(d.format), d.width, d.height,
                                                          static_cast<UINT16>(d.depthOrArraySize),
    ...
    chkDX(d3dDevice->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &rd,
                                              toD3D12States(d.initialState), cvPtr,
                                              IID_PPV_ARGS(&res)),
...
void Device::writeSrvForTexture(TextureRecord& tex)
{
    ...
    d3dDevice->CreateShaderResourceView(tex.resource.Get(), &sd,
                                        resourceHeap.cpuHandle(tex.srvIndex));
}
...
gfx::BufferHandle Device::createBuffer(const gfx::BufferDesc& d)
{
    ...
    auto rd = CD3DX12_RESOURCE_DESC::Buffer(d.size,
        gfx::any(d.usage, gfx::BufferUsage::UnorderedAccess)
            ? D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS
            : D3D12_RESOURCE_FLAG_NONE);

    ComPtr<ID3D12Resource> res;
    chkDX(d3dDevice->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &rd, initState, nullptr,
                                              IID_PPV_ARGS(&res)),
...
{
    if (!d.bytecode || d.bytecodeSize == 0) return {};
    std::lock_guard<std::mutex> lk(poolMu);
    uint32_t slot;
    ...
    rec.bytecode.assign(static_cast<const uint8_t*>(d.bytecode),
...
gfx::PipelineHandle Device::createGraphicsPipeline(const gfx::GraphicsPipelineDesc& d)
{
    ...
    if (!vs) throw std::runtime_error("createGraphicsPipeline: invalid vertex shader");
    ...
    if (ps) pd.PS = { ps->bytecode.data(), ps->bytecode.size() };
    ...
    pd.DepthStencilState.DepthWriteMask = d.depthStencil.depthWrite
    ...
    chkDX(d3dDevice->CreateGraphicsPipelineState(&pd, IID_PPV_ARGS(&pso)),
...
gfx::PipelineHandle Device::createComputePipeline(const gfx::ComputePipelineDesc& d)
{
    ...
    if (!cs) throw std::runtime_error("createComputePipeline: invalid compute shader");

    D3D12_COMPUTE_PIPELINE_STATE_DESC pd{};
    pd.pRootSignature = rootSignature.Get();
    pd.CS = { cs->bytecode.data(), cs->bytecode.size() };
    ComPtr<ID3D12PipelineState> pso;
    chkDX(d3dDevice->CreateComputePipelineState(&pd, IID_PPV_ARGS(&pso)),
...
gfx::SamplerHandle Device::createSampler(const gfx::SamplerDesc& d)
{
    auto toAddr = [](gfx::AddressMode m) {
        switch (m) {
            case gfx::AddressMode::Repeat: return D3D12_TEXTURE_ADDRESS_MODE_WRAP;
            case gfx::AddressMode::MirrorRepeat: return D3D12_TEXTURE_ADDRESS_MODE_MIRROR;
            case gfx::AddressMode::ClampToEdge: return D3D12_TEXTURE_ADDRESS_MODE_CLAMP;
            case gfx::AddressMode::ClampToBorder: return D3D12_TEXTURE_ADDRESS_MODE_BORDER;
        }
...
{
    if (h.id == 0 || h.id >= textures.size()) return;
    std::lock_guard<std::mutex> lk(poolMu);
    auto& rec = textures[h.id];
    if (!rec.external && rec.resource) {
        if (rec.srvIndex) resourceHeap.free(rec.srvIndex);
...
{
    if (h.id == 0 || h.id >= buffers.size()) return;
    std::lock_guard<std::mutex> lk(poolMu);
...
{
    if (h.id == 0 || h.id >= shaders.size()) return;
    std::lock_guard<std::mutex> lk(poolMu);
    shaders[h.id] = {};
...
{
    if (h.id == 0 || h.id >= pipelines.size()) return;
    std::lock_guard<std::mutex> lk(poolMu);
    pipelines[h.id] = {};
...
{
    if (h.id == 0 || h.id >= samplers.size()) return;
    std::lock_guard<std::mutex> lk(poolMu);
    if (samplers[h.id].index) samplerHeap_.free(samplers[h.id].index);
    samplers[h.id] = {};
...
void* Device::map(gfx::BufferHandle h)
{
    ...
    if (rec->mapped) return rec->mapped;
    D3D12_RANGE r{ 0, 0 };
...
void Device::uploadBuffer(gfx::BufferHandle dst, const void* data, uint64_t size,
                          uint64_t dstOffset)
{
    gfx::BufferDesc ud{};
    ud.size = size;
    ...
    if (!dstRec) throw std::runtime_error("uploadBuffer: invalid destination handle");
    ...
    if (dstRec->currentState != D3D12_RESOURCE_STATE_COPY_DEST) {
        auto b = CD3DX12_RESOURCE_BARRIER::Transition(dstRec->resource.Get(), dstRec->currentState,
                                                       D3D12_RESOURCE_STATE_COPY_DEST);
        native->ResourceBarrier(1, &b);
    }
    native->CopyBufferRegion(dstRec->resource.Get(), dstOffset, uploadRec->resource.Get(), 0, size);
    if (dstRec->currentState != D3D12_RESOURCE_STATE_COPY_DEST) {
        auto b = CD3DX12_RESOURCE_BARRIER::Transition(dstRec->resource.Get(),
                                                       D3D12_RESOURCE_STATE_COPY_DEST,
                                                       dstRec->currentState);
        native->ResourceBarrier(1, &b);
    }
