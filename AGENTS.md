...
- P13 (deferred) — `rootSignature`/`gridRootSig` ComPtrs (deferred to full bindless rewrite). BLAS/TLAS resources capability-gated. `spriteTexture` in BillboardRenderer stays ComPtr (no `adoptTexture` in gfx yet). Subsystem-owned descriptor heaps (bloom `bloomRtvHeap`, GBuffer `rtvHeap`, SSAO `rtvHeap`, shadow `dsvHeap`, picker, billboard) still ComPtr — deferred to P14.
- P14 — pending (blocked on full bindless rewrite to eliminate remaining D3D12 leaks in subsystems and Application root sigs).
