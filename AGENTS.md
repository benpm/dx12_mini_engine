...
- P13 (partial) ✅ — Application's scene PSO, gbuffer PSO, and grid PSO are `gfx::PipelineHandle`. Their VS/PS bytecodes go through `gfx::ShaderHandle`. The descriptor-table root sigs are passed via the new `GraphicsPipelineDesc::nativeRootSignatureOverride` escape hatch until P2. `gfx::IDevice` gained `nativeResource(TextureHandle)` / `nativeResource(BufferHandle)` accessors so engine subsystems can extract the underlying `ID3D12Resource*` for D3D12-specific calls (CreateDepthStencilView, custom-format SRVs).
...
**Known issue / deferred follow-up**: the engine's main depth buffer is `R32G8X24_TYPELESS` resource format with a `D32_FLOAT_S8X24_UINT` DSV; the scene/grid/gbuffer PSOs declare `D24_UNORM_S8_UINT` as DSVFormat. D3D12 tolerates this mismatch at runtime (debug-layer warning only), but trying to migrate the depth buffer through `gfx::createTexture` with an aligned format triggered a TDR on WARP — likely an interaction with the optimized clear-value handling for typeless formats. Reverted to keep depthBuffer as `ComPtr<ID3D12Resource>` until either (a) the bindless rewrite (P2) lets us redo the DSV format cleanly or (b) we add a separate "view format" / "clear format" field to `gfx::TextureDesc`.
