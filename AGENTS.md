...
- P13 (partial) ✅ — Application's scene/gbuffer/grid PSOs are `gfx::PipelineHandle` (use `nativeRootSignatureOverride`). VS/PS bytecodes go through `gfx::ShaderHandle`. The main `depthBuffer` and the cubemap color + depth textures are `gfx::TextureHandle`. `gfx::IDevice::nativeResource(handle)` returns the underlying `ID3D12Resource*` for D3D12-specific calls (CreateDepthStencilView, RTV/DSV array slices on a cubemap, custom-format SRVs).
- P2 (bindless shader rewrite) — postponed; high-risk and orthogonal to subsystem signature migration.
- P13 (remainder) — `dsvHeap`, `cubemapRtvHeap`, `cubemapDsvHeap` still ComPtr-typed; ditto the engine's main rootSignature ComPtr.
- P14 — pending.
